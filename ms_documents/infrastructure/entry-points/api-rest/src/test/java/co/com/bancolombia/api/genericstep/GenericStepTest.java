...
public class GenericStepTest {
    ...
    @Test
    public void finallyStepTest2(){
        InfoReuseCommon infoReuseCommon = InfoReuseCommon.builder()
                .dateResponseReuse(new Date()).responseReuse("asd")
                .dateRequestReuse(new Date()).requestReuse("asd").build();
        ...
        genericStep.finallyStep(stepForLogFunctional, UUID.randomUUID().toString(), infoReuseCommon, "asd");
        assertNotNull(stepForLogFunctional);
    }

    @Test
    public void finallyStepTest3(){
        InfoReuseCommon infoReuseCommon = InfoReuseCommon.builder()
                .dateResponseReuse(new Date()).responseReuse("asd")
                .dateRequestReuse(new Date()).requestReuse("asd").build();
        StepForLogFunctional stepForLogFunctional = StepForLogFunctional.builder()
                .webRequest(webRequest).acquisitionUseCase(acquisitionUseCase).fieldUseCase(fieldUseCase)
                .coreFunctionDate(coreFunctionDate).checkListUseCase(checkListUseCase).build();
        genericStep.finallyStep(stepForLogFunctional, null, infoReuseCommon, "asd");
        assertNotNull(stepForLogFunctional);
    }
    ...
    public void finallyStepNullAcquisitionTest() {
        InfoReuseCommon infoReuseCommon = InfoReuseCommon.builder()
                .dateResponseReuse(new Date()).responseReuse("asd")
                .dateRequestReuse(new Date()).requestReuse("asd").mapFields(new HashMap<>()).build();
        genericStep.finallyStep(null, infoReuseCommon, "asd");
        assertNotNull(stepForLogFunctional);
    }

    @Test
    public void finallyStepNullInfoReuseCommonTest() {
        doNothing().when(stepForLogFunctional).finallyStepForLogFunctional(any(StepForLogClass.class));
        genericStep.finallyStep(UUID.randomUUID().toString(), null, "asd");
        assertNotNull(stepForLogFunctional);
    }
