...
import static co.com.bancolombia.util.constants.Constants.ERROR_MSG_INCOMPLETE_FIELDS;
import static co.com.bancolombia.util.constants.Constants.ERROR_MSG_WRONG_UUID;

public class ErrorHandleRabbit {

    LoggerAdapter adapter = new LoggerAdapter(SYSTEM_VTN, SERVICE_MANAGEMENT, "RABBIT ERROR");

    protected void throwExceptionCodeWithoutLink() {
        HashMap<String, List<ErrorField>> errors = new HashMap<>();
        adapter.error(ERROR + SPACE + ERROR_CODE_WITHOUT_LINK);
        errors.put(ERROR_CODE_WITHOUT_LINK, Collections.singletonList(ErrorField.builder().build()));
