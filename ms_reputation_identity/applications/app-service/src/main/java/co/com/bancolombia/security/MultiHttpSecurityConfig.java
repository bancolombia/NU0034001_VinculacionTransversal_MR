...
public class MultiHttpSecurityConfig {
    ...
    public static class ProfileSecurityConfig extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity httpSecurity) {
            ...
            httpSecurity.authorizeRequests()
                    .antMatchers("/reputation-identity/api/v1/health").permitAll()
                    .and()
                    .antMatcher("/reputation-identity" + "/**/api/**").csrf().disable().exceptionHandling()
    ...
    public static class DocumentationSecurityConfig extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity httpSecurity) {
            ...
            httpSecurity.authorizeRequests().antMatchers("/reputation-identity/api/v1/health").permitAll()
                    .and().antMatcher("/reputation-identity" + "/**/api/**").csrf().disable().exceptionHandling()
                    .authenticationEntryPoint(new CustomAuthenticationEntryPoint())
                    .and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                    .and().addFilter(filter).authorizeRequests().anyRequest().authenticated()
                    .and().authorizeRequests();
